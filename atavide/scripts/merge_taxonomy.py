...
def parse_taxonomy(taxonfile, verbose=False):
    if not taxonfile:
        raise ValueError("Taxonomy file path is not provided or is empty.")
    if verbose:
...
if __name__ == "__main__":
     # These will be provided by Nextflow as environment variables
    tk = os.getenv('tk')
    lcafile = os.getenv('lca')
    taxa = os.getenv('taxonDB')
    outputfile = os.getenv('outputfile')

    taxa = parse_taxonomy(tk)
    write_lca(lcafile, taxa, outputfile)